...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
    <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
        ...
        <div style="background: #f9f9f9; padding: 30px; border-radius: 0 0 10px 10px;">
            <h2 style="color: #e74c3c; margin-top: 0;">✗ Document rejeté</h2>
            
            <p>Bonjour <strong>{{ $document->candidat->prenom }} {{ $document->candidat->nom }}</strong>,</p>
            
            <p>Nous vous informons que votre document n'a pas pu être validé.</p>
            ...
            <div style="background: white; padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #e74c3c;">
                ...
                <p style="margin: 10px 0 0 0;"><strong>Date de rejet :</strong> {{ $document->date_verification ? $document->date_verification->format('d/m/Y à H:i') : now()->format('d/m/Y à H:i') }}</p>
                @if($document->motif_rejet)
                <p style="margin: 10px 0 0 0;"><strong>Motif :</strong> {{ $document->motif_rejet }}</p>
                @endif
            </div>
            
            <p>Veuillez téléverser à nouveau un document conforme aux exigences.</p>
